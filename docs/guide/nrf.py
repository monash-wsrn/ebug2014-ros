...
class Bridge:
    ...
    def neighbour_discovery(self, index=0, only_new=False):
        ...
        for i in range(num_responses):
            ...
            if n != 8:
                raise RuntimeError('Unexpected response from bridge')
            r = self.usb.read(8)
            if r[0] != ('\xb0' if only_new else '\xb1'):
                raise RuntimeError('Unexpected response from bridge')
    ...
    def set_unicast_address(self, serial, address):
        ...
        if type(serial) is not str:
            serial = struct.pack('<BBBHBB', *serial)
        if type(address) is not str:
            address = chr(address)
    ...
    def load_device_info_from_file(self,  file):
        with open(file, 'r') as device_table_file:
            device_table = json.load(device_table_file)

        device_table = dict((int(key),  value) for key,  value in list(device_table.items()))
    ...
    def assign_static_addresses(self,  path='ebug_tab.json'):
        ...
        ebugs_psoc_id_list = dict((value['psoc_id'],  key) for key,  value in list(device_table.items()))
    ...
    def display_devices(self):
        ...
        print('----\t--------------------\t----------\t-------------------------------')
        for addr,  info in list(self.cameras.items()):
            print(addr,  '\t',  '-'.join([str(element) for element in info['psoc_id']]),  '\t',  'camera (1)')
        for addr,  info in list(self.ebugs.items()):
            print(addr,  '\t',  '-'.join([str(element) for element in info['psoc_id']]),  '\t',  'ebug (0)',  '\t',  str(info['led_sequence']))
        for psoc_id in self.unknowns:
            print('\t',  '-'.join([str(element) for element in psoc_id]),  '\t',  'UNKNOWN')
        print('----\t--------------------\t----------\t-------------------------------')
    ...
    def flash_all_ebugs(self, filename, which=None):
        ...
        Used by flash.py
        """
        for i, j in list(self.assign_addresses().items()):
    ...
    def print_laser_event(self):
        ...
        if e:
            ...
            print(
                '[%u] %3u: %5u - %5u (%u)'
                % (sensor_id, length, (pos-length) & 0xffff,
                   pos, ebug_id))
